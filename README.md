...
High performance Node.js (with native C addons) mining pool for Nolanium. Comes with lightweight example front-end script which uses the pool's AJAX API.
...
#### Requirements
* Latest [Nolanium Daemon](https://github.com/NolaniumProject/Nolanium)
...
```bash
git clone https://github.com/NolaniumProject/nolanium-nodejs-pool
...
#### 2) Create wallet
1. Go to the location where you extracted or compiled your Nolanium Daemon executables.
2. To create the wallet, use this command: `./Nolanium-Service --generate-container --container-file pool.wallet --container-password YourOwnPassword` (You can change YourOwnPassword or pool.wallet with your own information)
...
5. In order to start the Payment Processor (Wallet), use this command `./Nolanium-Service --generate-container --container-file pool.wallet --container-password YourOwnPassword --rpc-legacy-security`

**Note:** Do NOT open the `pool.wallet` file with `./Nolanium-Wallet`. This will break the wallet file from opening with `./Nolanium-Serivce`. If you did however do that, please restore your back in step 4.

_*We're working hard to migrate the pool from using `./Nolanium-Service` to `./Nolanium-Wallet-API`._
...
- **wallet -> host**: If you have your payment processor (Nolanium-Service) running somewhere else, you can change it to a different ip (Make sure to run the payment processor with `--bind-ip 0.0.0.0` in order to make a successful connection)
